            ...
            @else
                ...
                <div class="tab-content-item bg-white p-4 rounded shadow mb-4 relative">
                    ...
                    <div class="dropdown-content hidden">
                        ...
                        {{-- Lampiran button --}}
                        {{-- Lampiran button --}}
                        @if (!empty($help->lampiran) && file_exists(public_path('help-file/jaringan/' . $help->lampiran)))
                            @php
                            ...
                            @if (in_array(strtolower($fileExtension), ['mp4', 'avi', 'mov', 'mkv']))
                                ...
                                <div class="video-container mt-4">
                                    <video controls class="w-full max-w-4xl mx-auto">
                                        <source src="{{ asset('help-file/jaringan/' . $help->lampiran) }}" type="video/{{ $fileExtension }}">
                                        Your browser does not support the video tag.
                                    </video>
                                </div>
                            @else
                                <!-- If it's not a video, provide a download link -->
                                <a href="{{ asset('help-file/jaringan/' . $help->lampiran) }}" class="absolute bottom-4 right-4 inline-flex items-center px-4 py-2 text-sm font-medium text-blue-500 hover:bg-blue-100 rounded border">
