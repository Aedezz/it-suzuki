...
<div class="flex justify-center items-center mt-5">
    <div class="form-it-container w-full sm:w-11/12 lg:w-10/12 xl:w-11/12 2xl:w-3/4 bg-white rounded-lg shadow-md p-6 relative max-h-[80vh] overflow-auto">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('deskripsi.create', ['id_cabang' => request('id_cabang')]) }}" id="addButton"
        ...
        <form action="{{ route('deskripsi.index') }}" method="GET" class="flex space-x-4"
            style="margin-top: 25px; margin-bottom: 30px;">
            <!-- Filter inputs -->
            <select name="id_cabang" id="id_cabang" class="border rounded-md py-2 px-4 text-gray-900 text-sm" required>
                <option value="">Semua Cabang</option>
                @foreach ($cabang as $cb)
                    <option value="{{ $cb->id_cabang }}" {{ request('id_cabang') == $cb->id_cabang ? 'selected' : '' }}>
                        {{ $cb->nama_cabang }}
                    </option>
                @endforeach
            </select>

            <select name="username" id="username" class="border rounded-md py-2 px-4 w-72 text-gray-900 text-sm">
                <option value="">Pilih Nama</option>
                @foreach ($users as $user)
                    <option value="{{ $user->username }}"
                        {{ request('username') == $user->username ? 'selected' : '' }}>
                        {{ $user->nama }}
                    </option>
                @endforeach
            </select>
                        ...
                        <table id="example" class="display w-full table-auto border-collapse">
                            <thead>
                                <tr>
                                    ...
                                    <th class="py-2 px-4 border-b-2 text-center text-gray-800 font-semibold">Cabang
                                    </th>
                                    <th class="py-2 px-4 border-b-2 text-center text-gray-800 font-semibold">Tipe
                                    </th>
                                    <th class="py-2 px-4 border-b-2 text-center text-gray-800 font-semibold">Nama
                                        Deskripsi</th>
                            ...
                            <tbody>
                                ...
                                @else
                                    @foreach ($deskripsi as $index => $data)
                                        <tr>
                                            <td class="py-2 px-4 border-b text-center">{{ $index + 1 }}</td>
                                            <td class="py-2 px-4 border-b text-center">{{ $data->nama_cabang }}</td>
                                            <td class="py-2 px-4 border-b text-center">
                                                {{ $tipeAlias[$data->tipe] ?? 'Tipe Tidak Ditemukan' }}
                                            </td>
                                            <td class="py-2 px-4 border-b text-center">{{ $data->nama_deskripsi }}
                                            </td>
                                            <td class="py-2 px-4 border-b text-center">
                                            ...
                                            <td class="py-2 px-4 border-b text-center">
                                                <div class="flex justify-center space-x-2">
                                                    ...
                                                    <a href="{{ route('deskripsi.edit', $data->id_deskripsi) }}"
