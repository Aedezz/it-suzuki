            ...
            <div id='recipients' class="p-3 mt-6 lg:mt-0 rou    nded shadow bg-white"
                ...
                <div class="flex justify-between items-center px-4 py-8">
                    ...
                    <div class="flex space-x-6">
                        <a href="{{ route('ceklis.index') }}" class="text-gray-700 hover:text-indigo-500 transition-all duration-300">Ceklis</a>
                        <a href="{{ route('aktifitas.index') }}"
                            class="text-gray-700 hover:text-indigo-500 transition-all duration-300">Aktifitas</a>
                    </div>
                </div>
                <hr style="margin-top: -15px">
                <!-- Form Section -->
                <div class="container mt-4">
                    <form method="GET" action="{{ route('report.preview') }}">
                        <div class="form-row flex flex-wrap -mx-2">
                            <div class="mt-8 grid lg:grid-cols-2 gap-6 w-full">
                                ...
                                <div class="flex flex-col" style="margin-left: 20px">
                                    <label for="nama_periode" class="text-sm text-gray-700 font-medium mb-2">Periode</label>
                                    <select name="nama_periode" id="nama_periode" required
                                        ...
                                        @foreach ($periode as $item)
                                            <option value="{{ $item->id_periode }}" 
                                                @if ($item->id_periode == $defaultPeriode->id_periode) selected @endif>
                                                {{ $item->nama_periode }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <!-- Nama -->
                                <div class="flex flex-col" style="margin-left: 20px">
                                    <label for="nama" class="text-sm text-gray-700 font-medium mb-2">Nama</label>
